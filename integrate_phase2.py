...
# New function to show detailed cognitive analysis
  ...
  async function showDetailedCognitiveAnalysis(data, code) {
    ...
    content.innerHTML = `
      ...
      <div style="text-align: center; margin-bottom: 40px;">
        <h1 style="color: var(--text-primary); font-size: 2rem; margin-bottom: 10px;">📊 Detailed Cognitive Analysis</h1>
        <p style="color: var(--text-secondary);">Comprehensive breakdown of your coding session</p>
      </div>

      <!-- Test Results Section -->
      <div style="margin-bottom: 40px;">
        <h2 style="color: var(--text-primary); margin-bottom: 20px; font-size: 1.5rem;">✅ Test Results</h2>
        ${data.results.map((result, i) => `
          <div style="background: rgba(255,255,255,0.02); padding: 15px; border-radius: 10px; margin-bottom: 12px; border-left: 3px solid ${result.passed ? '#22c55e' : '#ef4444'};">
            <div style="display: flex; justify-content: space-between; align-items: center;">
              <div>
                <span style="color: var(--text-primary); font-weight: 600;">Test Case ${i + 1}</span>
                ${result.hidden ? '<span style="color: var(--text-secondary); margin-left: 10px; font-size: 0.85rem;">(Hidden)</span>' : ''}
              </div>
      ...
      ${cognitiveData ? `
        <div style="margin-bottom: 40px;">
          <h2 style="color: var(--text-primary); margin-bottom: 20px; font-size: 1.5rem;">🧠 Cognitive Insights</h2>
          
          <!-- Session Metrics -->
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-bottom: 30px;">
            <div style="background: rgba(255,255,255,0.03); padding: 20px; border-radius: 10px; text-align: center;">
              <div style="color: var(--text-secondary); font-size: 0.85rem; margin-bottom: 8px;">Coder Type</div>
              <div style="color: var(--accent1); font-size: 1.5rem; font-weight: 600;">${cognitiveData.analysis?.fingerprint?.type || 'Balanced'}</div>
            </div>
            <div style="background: rgba(255,255,255,0.03); padding: 20px; border-radius: 10px; text-align: center;">
              <div style="color: var(--text-secondary); font-size: 0.85rem; margin-bottom: 8px;">Typing Speed</div>
              <div style="color: var(--text-primary); font-size: 1.5rem; font-weight: 600;">${Math.round(cognitiveData.analysis?.metrics?.wpm || 0)} WPM</div>
            </div>
            <div style="background: rgba(255,255,255,0.03); padding: 20px; border-radius: 10px; text-align: center;">
              <div style="color: var(--text-secondary); font-size: 0.85rem; margin-bottom: 8px;">Focus Score</div>
            ...
            <div style="background: rgba(255,255,255,0.03); padding: 20px; border-radius: 10px; text-align: center;">
              <div style="color: var(--text-secondary); font-size: 0.85rem; margin-bottom: 8px;">Session Time</div>
              <div style="color: var(--text-primary); font-size: 1.5rem; font-weight: 600;">${Math.round((cognitiveData.analysis?.metrics?.total_time_sec || 0) / 60)}m</div>
            </div>
          </div>

          <!-- Cognitive Flow Chart -->
          <div style="background: rgba(255,255,255,0.02); padding: 20px; border-radius: 10px; margin-bottom: 20px;">
            <h3 style="color: var(--text-primary); margin-bottom: 15px; font-size: 1.2rem;">Typing Speed Over Time</h3>
          ...
          <div style="background: rgba(255,255,255,0.02); padding: 20px; border-radius: 10px;">
            <h3 style="color: var(--text-primary); margin-bottom: 15px; font-size: 1.2rem;">Coding State Timeline</h3>
            <div id="detailed-state-timeline" style="height: 60px; background: var(--bg2); border-radius: 8px; position: relative; overflow: hidden; margin-bottom: 15px;"></div>
      ...
      ` : `
        <div style="margin-bottom: 40px;">
          ...
          <div style="background: rgba(255,255,255,0.02); padding: 30px; border-radius: 10px; text-align: center;">
            <p style="color: var(--text-secondary);">Cognitive data not available for this session.</p>
            <p style="color: var(--text-secondary); font-size: 0.9rem; margin-top: 10px;">Start coding to see real-time cognitive analysis!</p>
          </div>
        </div>
      `}
      ...
      <!-- Performance Metrics -->
      <div style="margin-bottom: 40px;">
        <h2 style="color: var(--text-primary); margin-bottom: 20px; font-size: 1.5rem;">⚡ Performance Metrics</h2>
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
          <div style="background: rgba(255,255,255,0.03); padding: 25px; border-radius: 10px;">
            <div style="color: var(--text-secondary); font-size: 0.9rem; margin-bottom: 10px;">Runtime</div>
            <div style="color: var(--text-primary); font-size: 2rem; font-weight: 700;">${data.runtime}</div>
          </div>
          <div style="background: rgba(255,255,255,0.03); padding: 25px; border-radius: 10px;">
            <div style="color: var(--text-secondary); font-size: 0.9rem; margin-bottom: 10px;">Memory Usage</div>
            <div style="color: var(--text-primary); font-size: 2rem; font-weight: 700;">${data.memory}</div>
          </div>
        </div>
      </div>
...
try:
    ...
    replacement = detailed_analysis_function.strip() + '\\n\\n\\1'
    content = re.sub(pattern, replacement, content)
    
    # Update the "View Detailed Analysis" button handler in showSuccessScreen
    # Find the detailedBtn event listener
    old_handler = r"detailedBtn\.addEventListener\('click', \(\) => \{[\s\S]*?showSubmissionResults\(data\);[\s\S]*?\}\);"
    
    new_handler = '''detailedBtn.addEventListener('click', () => {
    ...
    print("Successfully integrated Phase 2 cognitive features into detailed analysis")
