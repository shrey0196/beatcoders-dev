...
class CRSCalculator:
    ...
    def calculate_crs(self, user_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        # ... (implementation same as before until tier determination)
        components = {
        ...
        # Simplify: Estimate days active from consistency data or pass it in
        # For now, we will rely on route passing it to _get_tier, but here we calculate it for the return value
        # In a real scenario, user_data should contain days_active
    ...
    def _get_tier(self, score: float, problems_solved: int = 0, days_active: int = 0) -> str:
        ...
        # Sort tiers by score threshold (highest first)
        sorted_tiers = sorted(
            self.TIERS.items(), 
            key=lambda x: x[1][0], 
            reverse=True
        ...
        for tier_name, (min_score, max_score) in sorted_tiers:
            # Check if score is in range (or above for highest tier)
            if score >= min_score:
